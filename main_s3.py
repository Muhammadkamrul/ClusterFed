...
def distribute_skewed_data_varying_sizes(clusters, train_dataset, num_classes=10, main_class_proportion=0.9):
    """
    Distributes data among clients, where each client gets a varying total number of samples, with 
    main_class_proportion of samples from one main class, and the rest from a diverse set of other classes.
    ...
    Args:
        ...
        min_samples: Minimum number of samples a client can receive.
        max_samples: Maximum number of samples a client can receive.
    ...
    for cluster in clusters:
        for client in cluster.clients:
            ...
            # Randomly determine the total number of samples for this client
            total_samples_for_client = np.random.randint(min_samples, max_samples + 1)

            main_class_samples = int(total_samples_for_client * main_class_proportion)  # Samples from the main class
            ...
            # Randomly add samples from the remaining classes, ensuring diversity
            other_classes = [i for i in range(num_classes) if i != main_class_id]
            np.random.shuffle(other_classes)  # Shuffle to select random classes

            # Calculate the number of samples to collect from the remaining classes
            remaining_samples = total_samples_for_client - main_class_samples
            samples_per_class = max(1, remaining_samples // (num_classes - 1))  # Split across multiple classes

            # Collect samples from different classes
            while len(client_indices[client_id]) < total_samples_for_client:
                for other_class_id in other_classes:
                    # Ensure we only sample from classes that still have available samples
                    if len(class_indices[other_class_id]) > 0:
                        sample_count = min(samples_per_class, len(class_indices[other_class_id]))
                        client_indices[client_id].extend(class_indices[other_class_id][:sample_count])
                        class_indices[other_class_id] = class_indices[other_class_id][sample_count:]  # Remove samples
                    # Break if we reach the required number of samples
                    if len(client_indices[client_id]) >= total_samples_for_client:
                        break

                # Check if there are enough samples remaining to continue; if not, break early
                if all(len(class_indices[c]) == 0 for c in other_classes):
                    break
...
def main():
    ...
    distribute_skewed_data_varying_sizes(clusters, train_dataset, main_class_proportion=0.7)
    ...
    with open('output_'+client_select_type+'_sel_s3.txt', 'w') as file:
