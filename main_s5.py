...
def distribute_shard_data(clusters, train_dataset):
    """
    Distributes data among clients by first sorting the entire train data by digit label, dividing it into 200 shards 
    of size 300, and assigning each client 2 random shards.
    ...
    assert num_clients * 2 <= 200, "Each client should receive 2 shards. Ensure there are no more than 100 clients."
    ...
    # Step 2: Flatten the list of indices (by concatenating each class's indices)
    for class_idx in range(num_classes):
        sorted_indices.extend(class_indices[class_idx])
    
    # Step 3: Divide the sorted data into 200 shards of size 300
    shard_size = 300
    num_shards = 200
    shards = [sorted_indices[i * shard_size:(i + 1) * shard_size] for i in range(num_shards)]
    
    # Step 4: Randomly assign 2 shards to each client
    client_indices = [[] for _ in range(num_clients)]
    shard_indices = np.random.permutation(num_shards)  # Randomize the shard assignments

    client_id = 0
    for cluster in clusters:
        for client in cluster.clients:
            # Each client gets 2 shards
            shard_1 = shard_indices[client_id * 2]
            shard_2 = shard_indices[client_id * 2 + 1]
            
            client_indices[client_id].extend(shards[shard_1])
            client_indices[client_id].extend(shards[shard_2])
...
def main():
    ...
    distribute_shard_data(clusters, train_dataset)
    ...
    with open('output_'+client_select_type+'_sel_s5.txt', 'w') as file:
