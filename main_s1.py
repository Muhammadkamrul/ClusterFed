...
weight = 0.5
D = 0.5 # Example deadline for transmission time (in seconds)
...
def distribute_one_class_per_client(clusters, train_dataset, num_classes=10):  
    ...
    # Assign one class to each client
    for cluster in clusters:
        for client in cluster.clients:
            class_id = client_id % num_classes
            # Limit the number of samples per client
            client_indices[client_id].extend(class_indices[class_id][:samples_per_client]) 
            client_id += 1
...
def main():
    ...
    distribute_one_class_per_client(clusters, train_dataset)
    ...
    with open('output_'+client_select_type+'_sel_s1.txt', 'w') as file:
